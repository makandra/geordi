Feature: Creating Rails database dumps with the "dumple" script
  ...
  Scenario: After dumping, run gzip to compress the dump in place
    Given a file named "config/database.yml" with:
    """
    development:
      adapter: mysql
  ...
  Scenario: Setting a custom compression algorithm for MySQL
    Given a file named "config/database.yml" with:
    """
    development:
      adapter: mysql
    ...
    When I run `dumple development --for_download --compress-algorithm=zstd`
    Then the output should match %r<system mysqldump -u"user" -p"password" --compression-algorithms=zstd  -r /home/.*/aruba/dumps/dump_for_download.dump --single-transaction --quick -hlocalhost>
  ...
  Scenario: Setting a custom compression algorithm for PostgreSQL
    ...
    When I run `dumple development --for_download --compress-algorithm=zstd:3`
