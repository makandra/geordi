Feature: The console command
  ...
  Scenario: Opening a local Rails console with an irb version between 1.2 and 1.2.6
    Given the irb version is "1.2"
  ...
  Scenario: Opening a remote Rails console with irb version >= 1.2.6
    Given the irb version is "1.2.6"
    And a file named "Capfile" with "Capfile exists"
    And a file named "config/deploy.rb" with "deploy file exists"
    And a file named "config/deploy/staging.rb" with:
    ...
    When I run `geordi console staging`
    Then the output should contain "# Opening a Rails console on staging"
    And the output should contain "Util.run! ssh, user@www.example.com, -t, cd /var/www/example.com/current && bash --login -c 'bundle exec rails console -e"
